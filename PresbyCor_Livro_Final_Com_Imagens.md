...
**Descrição:**  
Diagrama de corte transversal da córnea com camadas anatómicas e cálculo visual de RSB.

**Painel Esquerdo (Anatomia em Camadas):**

Corte sagital da córnea mostrando:
...
**Régua Lateral:**  
Escala em microns (0-550 μm) com marcações.

**Painel Direito (Cálculo Numérico):**

**Exemplo Paciente A (Seguro – Verde):**

```
Paquimetria Central:        550 μm
- Flap:                    -110 μm
...
**Exemplo Paciente B (Limítrofe – Amarelo):**

```
Paquimetria Central:        510 μm
- Flap:                    -110 μm
- Ablação Periférica:       -80 μm
─────────────────────────────────
= RSB:                      320 μm ⚠ (Limítrofe)

CONCLUSÃO: Considerar PRK (elimina flap) ou RLE
```

**Exemplo Paciente C (Inseguro – Vermelho):**

```
Paquimetria Central:        480 μm
- Flap:                    -110 μm
- Ablação Periférica:       -85 μm
─────────────────────────────────
= RSB:                      285 μm ✗ (<300 μm)

CONCLUSÃO: CONTRAINDICADO - RLE Indicada
```

**Linha Vermelha Horizontal:**  
Marca a zona dos 300 μm (limite de segurança biomecânico)

**Caixa de Alerta:**  
"**Regra Absoluta:** Nunca operar se RSB previsto <300 μm. Risco de ectasia iatrogénica inadmissível."

**Objetivo:**  
Ensinar visualmente o cálculo de RSB e a importância desta margem de segurança biomecânica.

---

Este Capítulo 3 está agora **completo e extenso**, pronto para ser copiado para o Google Drive! Continuamos com o Capítulo 4?
...
![Infográfico 5.2](/home/ubuntu/PresbyCor-Book/figures/chapter5/plot_qfactor_vs_add.png)
...
![Infográfico 11.3](/home/ubuntu/PresbyCor-Book/figures/chapter11/heatmap_complications.png)
...
## Apêndice A: Nomogramas Resumidos

### A.1. Nomograma PresbyCor/Custom-Q (Ghenassia)

**Q-Target em Função de Add Desejada**

| Add Desejada | Q-Target Olho Dominante | Q-Target Olho Não-Dominante |
|--------------|------------------------|----------------------------|
| +1.25 D | -0.50 | -0.60 |
| +1.50 D | -0.60 | -0.70 |
| +1.75 D | -0.70 | -0.80 |
...
$$S_{comp} = |\Delta Q| \times K_{médio} \times 0.75$$
...
- Q target = -0.70
- ΔQ = -0.45
- S_comp = 0.45 × 43 × 0.75 = **+14.5 D** (induzir hipermetropia para compensar)

---

### A.2. Nomograma PRESBYOND Simplificado

**Targets Automáticos (MEL 90)**

| Add Programada | OD (Dominante) | OE (Não-Dominante) | Anisometropia |
|----------------|----------------|-------------------|---------------|
| +1.50 D | +0.50 D | -1.00 D | 1.50 D |
| +1.75 D | +0.50 D | -1.15 D | 1.65 D |
| +2.00 D | +0.50 D | -1.25 D | 1.75 D |
...
### A.3. Nomograma PresbyMAX Hybrid

**Add Efetiva por Configuração**

| Add Programada | Perfil OD (Dominante) | Perfil OE (Não-Dom) | Add Efetiva Real |
|----------------|-----------------------|-------------------|------------------|
| +1.75 D | Monofocal (+0.25 D) | Bi-asférico (-0.50 D) | +1.60 D |
| +2.00 D | Monofocal (+0.25 D) | Bi-asférico (-0.75 D) | +1.85 D |
| +2.25 D | Monofocal (plano) | Bi-asférico (-1.00 D) | +2.10 D |

---

## Apêndice B: Tabelas de Conversão

### B.1. Snellen ↔ logMAR ↔ Decimal

| Snellen (20/x) | logMAR | Decimal | % Perda |
|----------------|--------|---------|---------|
| 20/10 | -0.30 | 2.00 | - |
| 20/12.5 | -0.10 | 1.60 | - |
| **20/15** | **-0.08** | **1.33** | - |
| **20/20** | **0.00** | **1.00** | **Normal** |
| 20/25 | +0.10 | 0.80 | 1 linha |
| 20/30 | +0.18 | 0.67 | 1.5 linhas |
| 20/40 | +0.30 | 0.50 | 2 linhas |
| 20/50 | +0.40 | 0.40 | 2.5 linhas |
| 20/60 | +0.48 | 0.33 | 3 linhas |
| 20/80 | +0.60 | 0.25 | 4 linhas |
| 20/100 | +0.70 | 0.20 | 4.5 linhas |
| 20/200 | +1.00 | 0.10 | 7 linhas |

---

### B.2. Conversão Jaeger ↔ Ponto ↔ logMAR (Perto)

| Jaeger | Ponto | logMAR | Descrição |
|--------|-------|--------|-----------|
| **J1** | 3 pt | 0.00 | Jornal small print |
| **J2** | 4 pt | 0.10 | Meta presbiópica |
| **J3** | 5 pt | 0.20 | Livro standard |
| J4 | 6 pt | 0.30 | Subótimo |
| J5 | 7 pt | 0.40 | Leitura difícil |
| J6 | 8 pt | 0.50 | Necessita óculos |

---

### B.3. Dioptria → Ablação (μm) - Aproximação

**Córnea Normal (K=43D):**

| Correção (D) | Ablação Central Aprox (μm) |
|--------------|------------------|
| -1.00 D (miopia) | ~12 μm |
| -3.00 D | ~36 μm |
| -6.00 D | ~75 μm |
| +1.00 D (hipermetropia) | ~15-20 μm periférico |
| +3.00 D | ~50-60 μm periférico |

**Nota:** Ablação presbiópica adiciona 30-60 μm conforme técnica (Custom-Q ~35 μm, PresbyMAX ~60 μm, SUPRACOR ~80 μm).

---

## Apêndice C: Checklists Cirúrgicas (1-Página)

### C.1. Checklist Pré-Op Universal

**Consulta Inicial:**
- [ ] Refração cicloplegiada (se <50 anos ou hipermétrope)
- [ ] Dominância ocular (hole-in-card + convergence)
- [ ] Add necessária (leitura trial com +1.50, +2.00, +2.50 D)
- [ ] Topografia corneana (Pentacam/Galilei)
- [ ] BAD-D ≤1.6 (screening ectasia)
- [ ] Paquimetria central >480 μm
- [ ] RSB estimado >320 μm
- [ ] Aberrometria (coma <0.30 μm, SA baseline)
- [ ] Avaliação cristalino: LOCS, OSI, densitometria
- [ ] OSDI (olho seco): <40
- [ ] TBUT >7 segundos
- [ ] Consentimento informado presbiópico assinado

---

### C.2. Checklist Intra-Op LASIK Presbiópico

**Preparação:**
- [ ] Paciente confirmado (nome, olho a operar)
- [ ] Cálculos verificados (Q-target, sphere, add)
- [ ] Laser programado e verificado
- [ ] Eye-tracker calibrado e testado

**Flap:**
- [ ] Marcação limbal (orientação)
- [ ] Criação flap (espessura target: 100-110 μm)
- [ ] Levantamento smooth
- [ ] Hidrodesidratação estromal (30-60 seg)

**Ablação:**
- [ ] Centragem verificada (Purkinje/Pupil conforme protocolo)
- [ ] Fixação paciente estável >5 segundos
- [ ] Tracking verde/ativo durante ablação
- [ ] Se tracking falha: **PARAR**

**Finalização:**
- [ ] Irrigação interface (BSS)
- [ ] Reposição flap com stretching
- [ ] Secagem margens (Merocel)
- [ ] Striae check (nenhuma visível)
- [ ] Antibiótico + NSAID tópico

---

### C.3. Checklist Follow-Up Pós-Op

**Dia 1:**
- [ ] UCDVA/UCNVA (baseline inicial)
- [ ] Refração (se UCVA <20/60, verificar)
- [ ] Flap check: Posição, sem striae, interface clara
- [ ] Prescrição: Antibiótico 4×/dia, NSAID 4×/dia, Lágrimas 6×/dia

**Semana 1:**
- [ ] UCDVA/UCNVA (melhoria esperada)
- [ ] Refração (estabilização inicial)
- [ ] Interface: DLK? (se sim, ver Capítulo 11)
- [ ] Gestão expectativas: "Visão estranha = normal, vai melhorar"

**Mês 1:**
- [ ] UCDVA/UCNVA (refraining from refraction se possível)
- [ ] Topografia (verify centragem)
- [ ] Decisão: Aguardar neuroadaptação vs. enhancement
- [ ] **Regra:** NÃO enhancement antes mês 6 exceto erro grosseiro

**Mês 3:**
- [ ] UCDVA/UCNVA (deve estar ≥80% final)
- [ ] Refração estável
- [ ] Satisfação score /10
- [ ] Se <6/10: Investigar causa (óptica vs neuroadaptativa)

**Mês 6-12:**
- [ ] Outcome final
- [ ] Alta ou enhancement se necessário

---

## Apêndice D: Templates de Consentimento Informado

### D.1. Consentimento Específico Presbiópico (Modelo)

```
CONSENTIMENTO INFORMADO - CIRURGIA REFRATIVA PARA PRESBIOPIA

Paciente: _________________________  Data: __________

Eu compreendo que a cirurgia presbiópica:

☐ Cria um COMPROMISSO entre visão de longe e perto  
☐ Minha visão de longe pode ser ligeiramente inferior ao que era antes  
☐ Vou experienciar halos/glare noturnos nos primeiros 3-6 meses  
☐ A neuroadaptação completa demora 3-6 meses  
☐ Posso precisar de óculos ocasionais (leitura prolongada, condução noturna)  
☐ 10-15% pacientes precisam de ajuste (enhancement)  
☐ 2-5% pacientes podem não se adaptar e necessitar reversão  
☐ Complicações raras incluem: infecção (<0.1%), ectasia (<0.5%), perda BCVA permanente (<0.3%)

Alternativas discutidas:
☐ Continuar com óculos/lentes de contacto  
☐ RLE (troca cristalino) - mais invasivo mas permanente  
☐ Outras técnicas presbiópicas (listadas: ____________)

Técnica escolhida: ☐ Custom-Q  ☐ PresbyMAX  ☐ PRESBYOND  ☐ Outra: _____

Add desejada: _____ D

Assinatura Paciente: ________________  
Assinatura Médico: ________________
```

---

### D.2. Checklist "Acordo de Expectativas" (Resumido)

```
ACORDO DE EXPECTATIVAS REALISTAS

Confirmo que compreendo:

✓ Não voltarei a ter visão "perfeita 20/15 longe  E J1 perto sem trade-offs"  
✓ Objetivo é FUNCIONALIDADE sem óculos >80% do tempo  
✓ Halos noturnos primeiros meses são NORMAIS (suprimem depois)  
✓ Devo ser PACIENTE durante neuroadaptação (mínimo 3 meses)  
✓ Vou EVITAR usar óculos antigos primeiras 4 semanas (atrapalha adaptação)  
✓ Se tiver dúvidas/ansiedade, vou CONTACTAR médico (não pesquisar internet ou pânico)
...
## Apêndice E: Tabela Master de Decisão Rápida

### Cenário Clínico → Técnica Recomendada

| Idade | DLS | Erro Refr | RSB | Add | **RECOMENDAÇÃO** |
|-------|-----|-----------|-----|-----|------------------|
| 47 | 1 | +2.50 D | 520 μm | +1.50 D | **Custom-Q LASIK** |
| 52 | 2 | Plano | 540 μm | +1.75 D | **PRESBYOND** (se Zeiss) ou Custom-Q |
| 54 | 2 | -1.50 D | 500 μm | +2.00 D | **PRESBYOND** ou PresbyMAX |
| 57 | 3 | -4.00 D | 480 μm | +2.25 D | **RLE (IOL EDOF/Trifocal)** |
| 62 | 3 | +1.00 D | 550 μm | +2.00 D | **RLE preferível** |
| 48 Pós-LASIK | 1 | +0.50 D | **310 μm** | +1.75 D | RLE (RSB limítrofe) |
| 55 | 2 | +3.50 D | 560 μm | +2.50 D | PresbyMAX ou RLE |
| 58 | 3 (OSI 2.5) | Plano | 530 μm | +2.00 D | **RLE** (cristalino comprometido) |

---

**FIM DOS APÊNDICES**

Estes apêndices fornecem **ferramentas de consulta rápida** para decisões clínicas diárias, consolidando informação dos 13 capítulos num formato prático e acessível.
