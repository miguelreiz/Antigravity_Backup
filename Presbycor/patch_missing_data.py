...
to_process = [r for r in records if not r.get("OD Equi Sphere (D)") or r.get("OD Equi Sphere (D)") == ""]
print(f"Found {len(to_process)} records missing Target Data.")
...
for i, rec in enumerate(to_process):
    ...
    if bname not in pdf_map:
        print(f"[{i+1}/{len(to_process)}] PDF not found: {source}")
        continue
    ...
    try:
        ...
        tokens_od.sort(key=lambda x: x[0])
        ...
        tokens_os.sort(key=lambda x: x[0])
        ...
        print(f"[{i+1}/{len(to_process)}] {source} | Updated | {elapsed:.1f}s | ETA: {eta:.1f}m")
        ...
        print(f"[{i+1}/{len(to_process)}] Error {source}: {e}")
...
print(f"Finished fixing {updated} records in {(time.time()-t0)/60:.1f}m!")
