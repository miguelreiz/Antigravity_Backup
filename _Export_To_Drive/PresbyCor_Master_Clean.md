...
*Figura 2.9: A "Zona de Cachinhos Dourados" da Aberração Esférica. O gráfico demonstra o trade-off entre ganho de profundidade de campo (curva verde) e perda de contraste (curva vermelha). A zona terapêutica segura (-0.40 a -0.60 μm) representa o equilíbrio ótimo onde benefícios excedem custos.*
...
### 2.3.5. Aplicação Prática em Cirurgia de Presbiopia

#### **Fluxograma de Triagem Pré-Operatória**

```
AVALIAÇÃO ABERROMÉTRICA PRÉ-OP
         ↓
    RMS HOA < 0.50 μm?
         ├─ NÃO → ❌ Irregularidade significativa
         │         → T-CAT regularização OU descontinuar
         └─ SIM → Avaliar Coma
                   ↓
              Coma (Z₃±¹) > 0.30 μm?
                   ├─ SIM → ⚠️ ALTO RISCO multifocal
                   │         → Opções:
                   │            1. Monovisão simples (sem SA negativa)
                   │            2. RLE (evitar córnea)
                   └─ NÃO → Avaliar SA pré-existente
                             ↓
                        SA total > +0.30 μm?
                             ├─ SIM → ⚠️ SA positiva alta
                             │         → Considerar DLS Stage 2
                             │         → Induzir Q hiper-prolato pode não compensar
                             └─ NÃO → ✅ CANDIDATO IDEAL
                                       → Prosseguir Custom-Q/PresbyCor
                                       → Alvo: SA -0.40 a -0.60 μm
                                       → Q-target: -0.80 a -1.20
```


#### **Caso Clínico 2: O Candidato Ideal**

**Apresentação:**
- Paciente: F, 52 anos
-Refração: OD +2.00 -0.50×180, OE +2.25 -0.75×5
- Queixa: "Uso óculos para tudo"

**Aberrometria (iTrace) - Olho Dominante (OD):**
- RMS HOA: **0.18 μm** ✅
- Coma: **0.10 μm** ✅
- SA: +0.08 μm (normal)
- Trefoil: 0.05 μm
...
**Topografia (Pentacam):**
- Q anterior: -0.28 (prolato normal)
- Paquimetria: 545 μm
- Sem sinais de ectasia

**Decisão Cirúrgica:**
✅ **Candidata PERFEITA para PresbyCor**

**Planejamento:**
- OD (dominante): Q-target **-1.00** (SA -0.50 μm), plano para longe
- OE (não-dominante): Q-target **-1.20** (SA -0.60 μm), -0.75 D para micro-monovisão
- Zona óptica: 6.5 mm (pupila mesópica 5.8 mm)
- Transição: 8.0 mm

**Resultado 12 meses:**
- OD: UDVA 20/20, UNVA 20/40
- OE: UDVA 20/25, UNVA 20/20
- **Binocular:** UDVA 20/20, UNVA 20/20 (J1)
- Q pós-op medido: OD -0.98, OE -1.18 (excelente precisão)
- Satisfação: 10/10
- **Lição:** Baixo RMS HOA pré-op prediz excelente resultado


### 2.3.7. Mensagens-Chave para o Cirurgião

> [!NOTE]
> **Resumo Executivo:**
> 1. **Coma pré-op > 0.30 μm** = Contraindicação para multifocal
> 2. **SA negativa** é sua ferramenta terapêutica → Alvo: -0.40 a -0.60 μm
> 3. **Zernike** = Padrão para presbiopia (wavefront-guided)
> 4. **Fourier** = Ferramenta para irregularidades (topography-guided)
> 5. **RMS HOA < 0.35 μm** pré-op prediz sucesso
> 6. **Centragem no Purkinje** é crítica para evitar indução de Coma
...
*Figura 2.7: Simulação de visão pré-operatória vs. pós-PresbyLASIK aos 6 meses. **Painel Esquerdo (Pré-op):** Visão longe perfeita (20/20) mas dependência total de óculos para perto e intermédio. **Painel Direito (Pós-op):** Independência funcional para perto (J2-J3) e intermédio com trade-off aceitável de halos noturnos moderados e discreta redução de contraste à distância (20/25). Esta ferramenta de simulação permite ao paciente tomar decisão INFORMADA sobre aceitabilidade do trade-off antes da cirurgia.*
...
4.  **Emétropes com Expectativas Irrealistas**
    -   Pacientes com visão de longe 20/15-20/20 que não toleram trade-off
